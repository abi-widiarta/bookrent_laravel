      ...
      <div class="bg-white/60 flex-1 space-y-4 w-full px-10 py-10 rounded-xl shadow-[0px_7px_50px_0px_rgba(198,203,232,0.2)]">
        ...
        <div class="flex relative items-center justify-between mb-12">
          <a class="flex items-center rounded-lg space-x-2 py-2 pl-2 pr-4 absolute top-0 left-0 bg-[#FFE8E8] text-[#FF5050] text-sm font-semibold" href="/admin/books">
            ...
            <p>Back</p></a>
          <h1 class="text-[#151C48] font-semibold text-2xl text-center  w-full">Edit Book</h1>
        </div> 
        {{-- Form --}}
        <div class="px-10">
            ...
            <div class="flex divide-x-2 px-10 mb-8">
              <div class="w-1/2 pr-8 space-y-6 ">
                <div class="flex flex-col space-y-2 items-start">
                  ...
                  <input class="p-2 focus:outline-[#777A8F]/80 text-[#777A8F]/90 text-sm  rounded-lg w-full border border-[#777A8F]/20" value="{{ $book->title }}" type="text" name="title" id="title">
                </div>
                <div class="flex flex-col space-y-2 items-start">
                  <label class="text-[#777A8F] font-medium text-sm" for="title">Author</label>
                  <input class="p-2 focus:outline-[#777A8F]/80 text-[#777A8F]/90 text-sm  rounded-lg w-full border border-[#777A8F]/20" value="{{ $book->author }}" type="text" name="author" id="author">
                </div>
                <div class="flex flex-col space-y-2 items-start">
                  <label class="text-[#777A8F] font-medium text-sm" for="title">Category</label>
                  <select class="js-example-basic-multiple p-2 focus:outline-[#777A8F]/80 text-[#777A8F]/90 text-sm  rounded-full w-full" name="categories[]" value={{ $book->categories }} multiple="multiple">
                    @foreach ($categories as $category)
                      <option value={{ $category->id }}>{{ $category->name }}</option>
                    @endforeach
                  </select>
                  <p class="text-xs text-black/50"> Current category : 
                ...
                <div class="flex flex-col space-y-2 items-start">
                  ...
                  <div class="flex space-x-8">
                    ...
                    <div class="flex items-center space-x-2">
                      <input class="border borde-[#777A8F] accent-[#777A8F]" {{ $book->status == 'In stock' ? 'checked' : '' }}  type="radio" id="in_stock" name="status" value="In stock">
                      <label class="text-sm text-[#777A8F]" for="in_stock">In Stock</label><br>
                    </div>
                    <div class="flex items-center space-x-2">
                      <input class="border borde-[#777A8F] accent-[#777A8F]" {{ $book->status == 'Out Of Stock' ? 'checked' : '' }} type="radio" id="out_of_stock" name="status" value="Out Of Stock">
              ...
              <div class="w-1/2 pl-8">
                <div class="flex flex-col space-y-2 items-start h-full">
                  ...
                  <textarea class="resize-none leading-6 font-normal w-full h-full px-4 py-2 text-sm text-[#777A8F] rounded-lg  border border-[#777A8F]/20 focus:outline-[#777A8F]/50" name="description">{{ $book->description }}</textarea>
                  {{-- <textarea  type="text" name="title" id="title"> --}}
                </div>
              </div>
  
            </div>
            <button type="submit" class=" block mx-auto px-24 shadow-[0px_4px_4px_0px_rgba(255,55,55,0.23)] hover:shadow-[0px_6px_6px_0px_rgba(255,55,55,0.3)] hover:opacity-80 hover:-translate-y-1 transition-all duration-300 origin-left py-2.5 rounded-xl bg-[#FF3737] text-white font-medium text-sm">Edit Book</button>
          </form>
        </div>
      </div>
    </div>
